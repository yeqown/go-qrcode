...
func analyzeJP(r rune) bool {
	_, ok := __UNICODE_TO_QR_KANJI[uint32(r)]
	return ok
}

var (
	__UNICODE_TO_QR_KANJI map[uint32]struct{}
)
...
func restoreKanJi() {
	...
	__UNICODE_TO_QR_KANJI = make(map[uint32]struct{}, 1<<16)
	// restore from __PACKED_QR_KANJI_TO_UNICODE
	for i := 0; i < len(__PACKED_QR_KANJI_TO_UNICODE); i += 2 {
		c := (uint32(__PACKED_QR_KANJI_TO_UNICODE[i]) << 8) | uint32(__PACKED_QR_KANJI_TO_UNICODE[i+1])
		if c == 0xFFFF {
			continue
		}
		__UNICODE_TO_QR_KANJI[c] = struct{}{}
	}
}

var __PACKED_QR_KANJI_TO_UNICODE = "MAAwATAC/wz/DjD7/xr/G/8f/wEwmzCcALT/QACo/z7/4/8/MP0w/jCdMJ4wA07dMAUwBjAHMPwgFSAQ/w8AXDAcIBb/XCAmICUgGCAZIBwgHf8I/wkwFDAV/zv/Pf9b/10wCDAJMAowCzAMMA0wDjAPMBAwEf8LIhIAsQDX//8A9/8dImD/HP8eImYiZyIeIjQmQiZA" +
