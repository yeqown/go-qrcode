...
import (
	...
	yeqownwstd "github.com/yeqown/go-qrcode/writer/standard"
)
...
qr-yeqown.png:      1258  // yeqown default compression level
qr-yeqown-best.png: 1215  // yeqown best compression level
*/
func main() {
	// some json
...
func encodeWithYeqown(content, name string) {
	...
	stdw, err := yeqownwstd.New(name,
		yeqownwstd.WithBuiltinImageEncoder(yeqownwstd.PNG_FORMAT),
		yeqownwstd.WithQRWidth(1),
		yeqownwstd.WithBorderWidth(4),
		//yeqownwstd.WithCompressed(),
	)
	if err != nil {
		panic(err)
	}
...
func encodeWithYeqownCompression(content, name string, imageEncoder CustomPngEncoder) {
	...
	stdw, err := yeqownwstd.New(name,
		yeqownwstd.WithCustomImageEncoder(imageEncoder),
		yeqownwstd.WithQRWidth(1),
		yeqownwstd.WithBorderWidth(4),
		yeqownwstd.WithCompressed(),
	)
